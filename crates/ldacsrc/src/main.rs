...
fn main() {
    ...
    while let Some(arg) = iter.next() {
        match arg.as_str() {
            ...
            "--capture" => {
                capture_device = Some(
                    iter.next()
                        .expect("--capture needs a device name fragment")
                        .clone(),
                );
            }
            ...
            "--help" | "-h" => {
                println!(
                    "ldacsrc [--addr AA:BB:CC:DD:EE:FF] [--quality hq|sq|mq] [--abr] [--capture NAME] [--auto-bt-fallback SECS] [--check-capture] [--list-capture]"
                );
                println!("without --addr the first audio sink found by inquiry is used");
                println!(
                    "without --capture the default playback device is captured; --list-capture shows names"
                );
                println!(
                    "--auto-bt-fallback SECS hands the radio back to Windows (via ldacmode.ps1, UAC) once"
                );
                println!(
                    "a stream was established and then lost for SECS seconds; 0 (default) disables"
                );
                return;
            }
    ...
    if codec::freq_bit(format.sample_rate).is_none() {
        ...
        eprintln!(
            "set the shared-mode format of the default playback device to 44.1/48/88.2/96 kHz."
        );
        std::process::exit(2);
    }
    ...
    if !matches!(format.channels, 1 | 2) {
        eprintln!(
            "LDAC requires mono or stereo capture, got {} channels",
            format.channels
        );
        std::process::exit(2);
    }
...
unsafe fn setup_btstack() {
    unsafe {
        ...
        let offered = LdacCaps {
            ...
            channel_modes: if app().format.channels == 1 {
                codec::CHAN_MONO
            } else {
                codec::CHAN_STEREO
            },
...
unsafe extern "C" fn hci_handler(packet_type: u8, _channel: u16, packet: *mut u8, _size: u16) {
    unsafe {
        ...
        match bt::hci_event_packet_get_type(packet) as u32 {
            bt::BTSTACK_EVENT_STATE => {
                if bt::btstack_event_state_get_state(packet) as i32
                    != bt::HCI_STATE_HCI_STATE_WORKING
                {
                    return;
                }
                let mut local = [0u8; 6];
                ...
                let tlv = bt::btstack_tlv_windows_init_instance(
                    &mut a.tlv_ctx,
                    a.tlv_path.as_ref().unwrap().as_ptr(),
                );
                bt::btstack_tlv_set_instance(
                    tlv,
                ...
                bt::btstack_run_loop_set_timer_handler(
                    &mut a.stats_timer,
                    Some(stats_timer_handler),
                );
            ...
            bt::GAP_EVENT_DEDICATED_BONDING_COMPLETED => {
                ...
                if status as u32 != bt::ERROR_CODE_SUCCESS {
                    ...
                    println!(
                        "bonding with {} exited {status:#04x}, trying audio",
                        addr_str(&addr)
                    );
                } else {
                ...
                bt::btstack_run_loop_set_timer_handler(
                    &mut a.audio_delay_timer,
                    Some(audio_delay_timer_handler),
                );
...
unsafe fn schedule_reconnect() {
    unsafe {
        ...
        bt::btstack_run_loop_set_timer_handler(
            &mut a.reconnect_timer,
            Some(reconnect_timer_handler),
        );
...
unsafe extern "C" fn reconnect_timer_handler(_t: *mut bt::btstack_timer_source_t) {
    unsafe {
        establish();
    }
}

unsafe extern "C" fn audio_delay_timer_handler(_t: *mut bt::btstack_timer_source_t) {
    unsafe {
        establish_audio();
    }
}

unsafe extern "C" fn a2dp_handler(packet_type: u8, _channel: u16, packet: *mut u8, _size: u16) {
    unsafe {
        if packet_type != bt::HCI_EVENT_PACKET as u8 {
            return;
        }
        ...
        match bt::hci_event_a2dp_meta_get_subevent_code(packet) as u32 {
            bt::A2DP_SUBEVENT_SIGNALING_CONNECTION_ESTABLISHED => {
                ...
                a.a2dp_cid =
                    bt::a2dp_subevent_signaling_connection_established_get_a2dp_cid(packet);
                a.sink_caps = None;
            ...
            bt::A2DP_SUBEVENT_SIGNALING_MEDIA_CODEC_OTHER_CAPABILITY => {
                let remote_seid =
                    bt::a2dp_subevent_signaling_media_codec_other_capability_get_remote_seid(
                        packet,
                    );
            ...
            bt::A2DP_SUBEVENT_SIGNALING_CAPABILITIES_COMPLETE => {
                ...
                let chosen =
                    match codec::choose_config(caps, a.format.sample_rate, a.format.channels) {
                        Ok(c) => c,
                        Err(e) => {
...
unsafe fn request_can_send(a: &mut App) {
    unsafe {
...
unsafe extern "C" fn audio_timer_handler(_t: *mut bt::btstack_timer_source_t) {
    unsafe {
        let a = app();
        bt::btstack_run_loop_set_timer(&mut a.audio_timer, AUDIO_TIMER_MS);
        bt::btstack_run_loop_add_timer(&mut a.audio_timer);
        ...
        while a.audio.len() >= a.pcm.len() && budget > 0 && a.ready.len() < READY_MAX {
            ...
            a.stats.samples_consumed +=
                (out.pcm_used / (channels * std::mem::size_of::<f32>())) as u64;
            ...
            a.pending_samples += out.frames as u32
                * if a.format.sample_rate > 48_000 {
                    256
                } else {
                    128
                };
        ...
        if let Some(done) = a.packer.as_mut().unwrap().flush() {
            a.ready
                .push_back((done, std::mem::take(&mut a.pending_samples)));
        }
...
unsafe extern "C" fn stats_timer_handler(_t: *mut bt::btstack_timer_source_t) {
    unsafe {
        let a = app();
        bt::btstack_run_loop_set_timer(&mut a.stats_timer, STATS_TIMER_MS);
        bt::btstack_run_loop_add_timer(&mut a.stats_timer);
        ...
        let bytes_per_frame = a
            .stats
            .payload_bytes
            .checked_div(a.stats.frames)
            .unwrap_or(0);
        ...
        let alive = matches!(
            a.state,
            State::Bonding | State::Connecting | State::Configuring | State::Streaming
        );
        let fallback_due = a.streamed_once
            && !alive
            && a.fallback_after_s != 0
            && now.saturating_sub(a.stream_lost_ms) > a.fallback_after_s * 1000
            && !auto_bt_fallback();
        if !a.streamed_once || alive || fallback_due {
